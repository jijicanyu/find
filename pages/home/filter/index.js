...
data: {
  ...
  CustomBar: app.globalData.CustomBar,
	...
	sex:'',
	selectdIndex:0,
	sexArr:[{label:'选择全部',value:''},{label:'只看男生',value:'0'},{label:'只看女生',value:'1'}],
  multiIndex:[0,0],
  ...
  confirmAgeModal(){
    const that = this;
     that.setData({
      minAge:that.data.lowAge,
      maxAge:that.data.heighAge,
      ageModal:null
    })
  },
  hideAgeModal(e) {
  ...
  lowAgeValueChangeAction1: function (e) {
    this.setData({
      lowAge: e.detail.lowValue
    })
  },

  heighAgeValueChangeAction1: function (e) {
    this.setData({
      heighAge: e.detail.heighValue
    })
  },
  ...
  confirmHeightModal(){
    const that = this;
     that.setData({
      minHeight:that.data.lowHeight,
      maxHeight:that.data.heighHeight,
      heightModal:null
    })
  },
  hideHeightModal(e) {
  ...
  lowHeightValueChangeAction1: function (e) {
    this.setData({
      lowHeight: e.detail.lowValue
    })
  },

  heighHeightValueChangeAction1: function (e) {
    this.setData({
      heighHeight: e.detail.heighValue
    })
  },
  ...
  chooseMasterCheckbox(e) {
    let items = this.data.checkMasterBox;
    ...
    this.setData({
      checkMasterBox: items,
    })
  },
  confirmMasterModal(){
    const that = this;
     that.setData({
      masterCheckedArr:that.data.checkMasterBox.filter(item=>item.checked === true),
      masterModal:null
    })
  },
  hideMasterModal() {
  ...
  chooseSchoolCheckbox(e) {
    let items = this.data.checkSchoolBox;
    let values = e.currentTarget.dataset.value;
    for (let i = 0, lenI = items.length; i < lenI; ++i) {
      if (items[i].value == values) {
        items[i].checked = !items[i].checked;
    ...
    this.setData({
      checkSchoolBox: items,
    })
  },
  confirmSchoolModal(){
    const that = this;
     that.setData({
      schoolCheckedArr:that.data.checkSchoolBox.filter(item=>item.checked === true),
      schoolModal:null
    })
  },
  hideSchoolModal() {
  ...
  onLoad: function (option) { 
    ...
    if(option.from === 'list'){
    	for (let i = 0; i < that.data.sexArr.length; i++) {
	    	if(that.data.sexArr[i].value == sex){
	    		that.setData({
            disableChange:true,
            selectdIndex:i,
            from:'list'
  ...
  sexChange:function(e){
  	...
  	that.setData({
  		...
  		selectdIndex:e.currentTarget.dataset.index,
  	})
  },
  dissableSexChange:function(e){
  	console.log(e)
  },
  // 所在城市
  bindMultiPickerChange: function(e){
    ...
    that.setData({
      ...
      region:[that.data.multiArray[0][that.data.multiIndex[0]],that.data.multiArray[1][that.data.multiIndex[1]]]
    })
  },
  bindMultiPickerColumnChange: function (e){
    ...
    switch (e.detail.column){
      case 0:
        let list = []
        for (let i = 0; i < that.data.placeArr.length;i++){
          if (that.data.placeArr[i].parid == that.data.placeArr[e.detail.value].regid){
            list.push(that.data.placeArr[i].regname)
          }
        }
        that.setData({
          "multiArray[1]": list,
  ...
  //   const that = this;
  //   var age = ''
  //   for (let i = 0; i < that.data.ages.length; i++) {
  //     if(i === Number(e.detail.value)){
  //       age = that.data.ages[i].name;
  //       that.setData({
  ...
  //     if(i === Number(e.detail.value)){
  //       height = that.data.heights[i].name;
  //       that.setData({
  ...
  //   const that = this;
  //   var school = ''
  //   for (let i = 0; i < that.data.schools.length; i++) {
  //     if(i === Number(e.detail.value)){
  //       school = that.data.schools[i].id;
  //       that.setData({
    ...
    const data = {
    	sex:that.data.sex,
     	region:that.data.region,
      	minHeight:that.data.minHeight,
      	maxHeight:that.data.maxHeight,
      	minAge:that.data.minAge,
      	maxAge:that.data.maxAge,
      	education:that.data.masterCheckedArr,
      	school:that.data.schoolCheckedArr,
    }
    console.log(data)    
    if(that.data.from !== 'list'){
    	wx.setStorageSync('sex', that.data.sex);
    }
  	// app.globalData.user.sex = that.data.sex;
  	console.log(app.globalData)
    if (wxPrevPage) {
      console.log(wxPrevPage)
      wxPrevPage.setData({
        filterData: JSON.stringify(data) // baseData为上级页面的某个数据
      })
    }
    // wx.navigateBack();
    wx.switchTab({
